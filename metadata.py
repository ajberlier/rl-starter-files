...
class TrainingStats():
    ...
    def append_accumulate_dict(self, metric_dict: dict[str, any]):
        for k, v in metric_dict.items():
            self.append_accumulate(k, v)
    ...
    def close_all_accumulate(self, epoch, method: str = 'mean'):
        metric_names = list(self.accumulator.keys())
        for metric_name in metric_names:
            self.close_accumulate(epoch, metric_name, method)
    ...
    def add_dict(self, epoch: int, metric_dict: dict[str, any]):
        for k, v in metric_dict.items():
            self.add(epoch, k, v)
